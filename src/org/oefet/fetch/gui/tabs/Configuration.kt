...
object Configuration : Grid("Configuration", 1) {
    ...
    val ground = Configurator.SMU(
        "Ground Channel (for SPA)",
        Settings.groundConfig,
        Connections
    )

    val sourceDrain = Configurator.SMU(
        "Source-Drain Channel",
        Settings.sourceDrainConfig,
        Connections
    )

    val sourceGate = Configurator.SMU(
        "Source-Gate Channel",
        Settings.sourceGateConfig,
        Connections
    )

    val fourPP1 = Configurator.VMeter(
        "Four Point Probe Channel 1",
        Settings.fourPP1Config,
        Connections
    )

    val fourPP2 = Configurator.VMeter(
        "Four Point Probe Channel 2",
        Settings.fourPP2Config,
        Connections
    )

    val tControl = Configurator.TC(
        "Temperature Control",
        Settings.tControlConfig,
        Connections
    )

    val tMeter = Configurator.TMeter(
        "Temperature Sensor",
