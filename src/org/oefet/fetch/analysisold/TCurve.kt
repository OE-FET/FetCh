...
import org.oefet.fetch.analysisold.Curve.Companion.NON_USER_VARIABLES
...
class TCurve(private val results: ResultTable) : Curve {
    ...
    init {
        ...
        fppSeparation       = results.getAttribute("FPP Separation").removeSuffix(" m").toDouble()
        channelThickness    = results.getAttribute("Thickness").removeSuffix(" m").toDouble()
        dielectricThickness = results.getAttribute("Dielectric Thickness").removeSuffix(" m").toDouble()
        permittivity        = results.getAttributeDouble("Dielectric Permittivity")
