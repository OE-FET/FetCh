...
class TVHighThroughput :
    ...
    private val pctMarginPeltiers by userInput(
        "Temperature Stabilization: Peltiers",
        "Percentage range for temperature to stay within",
        0.3
    )
    private val durationPeltiers by userInput(
        "Temperature Stabilization: Peltiers",
        "Duration of temperature stabilization [s]",
        60.0
    ) map { (it * 1e3).toLong() }
    private val maxTimePeltiers by userInput(
        "Temperature Stabilization: Peltiers",
        "Maximum Duration of temperature stabilization [s]",
        180.0
    ) map { (it * 1e3).toLong() }

    private val pctMarginOnChip by userInput(
        "Temperature Stabilization: On chip",
        "Percentage range for temperature to stay within",
        1.0
    )
    private val durationOnChip by userInput(
        "Temperature Stabilization: On chip",
        "Duration of temperature stabilization [s]",
        10
    ) map { (it * 1e3).toLong() }
    private val maxTimeOnChip by userInput(
        "Temperature Stabilization: On chip",
        "Maximum Duration of temperature stabilization [s]",
        180.0
    ) map { (it * 1e3).toLong() }
    ...
    override fun run(results: ResultTable) {
        ...
        for (temperatures in list) {
            ...
            coldPeltier.setPoint = temperatures.cold
            hotPeltier.setPoint  = temperatures.hot
            ...
            runInParallel(
                {
                    coldPeltier.waitForStableTemperature(
                        temperatures.cold,
                        pctMarginPeltiers,
                        durationPeltiers
                    )
                ...
                {
                    hotPeltier.waitForStableTemperature(
                        temperatures.hot,
                        pctMarginPeltiers,
                        durationPeltiers
                    )
            ...
            results.mapRow(
                ...
                VOLTAGE_ERROR to determineVoltageStdDeviation(
                    vMeter1Values.standardDeviation,
                    vMeter2Values.standardDeviation,
                    ground?.voltage ?: Double.NaN
                ),
    ...
    override fun onFinish() {
        ...
        runRegardless(
            ...
            { vMeter2?.turnOff() },
            { ground?.turnOff() },
            { hotPeltier.manualValue  = 0.0; hotPeltier.isPIDEnabled = false },
            { coldPeltier.manualValue = 0.0; hotPeltier.isPIDEnabled = false }
        )
